...
OUT_ROOT = Path.home() / "bop_datasets" / "ycbv_3dsgrasp_eval_filtered"
...
def filter_depth_outliers(points):
    """
    Robust filter for masked RGB-D object points.
    Uses only the partial point cloud itself, no GT model.
    """
    ...
    if robust_sigma < 1.0:
        robust_sigma = np.std(z)

    z_window = max(200.0, 4.0 * robust_sigma)
    keep_z = np.abs(z - z_med) < z_window
...
def main():
    ...
    for scene_id, im_id in tqdm(sorted(scene_to_im.items())):
        ...
        for gt_idx, gt_entry in enumerate(gt_entries):
            ...
            partial_points_raw = depth_mask_to_points(depth, mask, cam_K, depth_scale)
            partial_points = filter_depth_outliers(partial_points_raw)

            if len(partial_points) < 50:
                print(
                    f"[WARN] Too few filtered partial points: "
                    f"scene={scene_id}, im={im_id}, gt_idx={gt_idx}, "
                    f"raw={len(partial_points_raw)}, filtered={len(partial_points)}"
                )
                continue
            ...
            rows.append({
                ...
                "num_partial_points_raw_before_filter": len(partial_points_raw),
            })
